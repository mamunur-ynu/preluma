...
# Only topics that exist in TOPICS dict
TOPIC_OPTIONS = [
    "Quantum Mechanics",
    "Machine Learning",
    "Python Programming",
...
def sidebar():
    ...
    st.sidebar.caption("Python + Streamlit + Wikipedia real data upgrade.")
...
def chips():
    ...
    st.markdown(
        " ".join(f"<span class='chip'>{c}</span>" for c in labels),
        unsafe_allow_html=True,
    )
...
# ─────────────────────────────────────────────
# Student Mission page sections
# ─────────────────────────────────────────────

def mission_control():
    st.markdown("### Mission Control")
    st.markdown(
        "<div class='notice'>Choose a topic. Preluma will generate a simple pre-class "
        "learning mission powered by Python and, if configured, the Claude AI API.</div>",
        unsafe_allow_html=True,
    )
    ...
    with st.form("mission_form", border=True):
        ...
        with c1:
            ...
            topic = (
                st.text_input("Enter your topic", value="", placeholder="e.g. Reinforcement Learning")
        ...
        with c3:
            ...
            for item in ["Tiny answer", "Simple explanation", "Real example",
                         "Mistake correction", "Class questions"]:
                st.caption(item)
...
def brain_brief():
    ...
    st.markdown(
        ...
        <div class='answer-card'><div class='answer-title'>Key concept</div>
          <p><b>{b['key_concept']}</b>: {b['concept_simple']}</p></div>
...
def quiz():
    ...
    st.caption("Answer once. Preluma will detect weak skills and explain mistakes.")
...
def result_section():
    ...
    for i, d in enumerate(result["details"], 1):
        ...
        with st.expander(f"Question {i}: {status} — {d['skill']}"):
            ...
            if not d["correct"]:
                st.info("Quick fix: read the definition, connect it to one example, then say it in your own words.")
...
def tutor():
    ...
    # Show whether Claude API is active
    try:
        from llm import llm_available
        if llm_available():
            st.markdown(
                "<div class='notice'>Claude AI is active. UltraTutor will answer with real AI intelligence.</div>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                "<div class='notice'>UltraTutor is running on local concept data. "
                "Set ANTHROPIC_API_KEY to enable Claude AI answers.</div>",
                unsafe_allow_html=True,
            )
    except ImportError:
        st.markdown(
            "<div class='notice'>If one idea is confusing, ask here. "
            "Tutor will answer with tiny answer, example, mistake, and exam angle.</div>",
            unsafe_allow_html=True,
        )
    ...
    if st.button("Explain Clearly", use_container_width=True):
        ...
        st.markdown(
            f"""
            <div class='answer-card'><div class='answer-title'>Tiny answer</div>
              <p>{s.get('tiny_answer','')}</p></div>
            <div class='answer-card'><div class='answer-title'>Explain simply</div>
              <p>{s.get('explain_simply','')}</p></div>
            <div class='answer-card'><div class='answer-title'>Real-life example</div>
              <p>{s.get('real_life_example','')}</p></div>
            <div class='answer-card'><div class='answer-title'>Common mistake</div>
              <p>{s.get('common_mistake','')}</p></div>
            <div class='answer-card'><div class='answer-title'>Exam angle</div>
              <p>{s.get('exam_angle','')}</p></div>
            """,
            unsafe_allow_html=True,
        )
...
def questions_download():
    ...
    for i, q in enumerate(st.session_state.pack["class_questions"], 1):
        st.write(f"{i}. {q}")
...
def teacher_studio():
    ...
    st.write(
        "Teacher value: the teacher can see who is ready, who is confused, "
        "and what topic needs a quick warm-up."
    )
...
def evidence_board():
    ...
    st.markdown(
        ...
        <div class='prof-grid'>
          <div class='prof-card'><h4>Clear Problem</h4>
            <p>Students often enter lectures unprepared, which leads to passive learning and poor retention.</p></div>
          <div class='prof-card'><h4>Python Implementation</h4>
            <p>The project is built with Python, Streamlit, Pandas, Plotly, dictionaries, functions, forms, and session state.</p></div>
          <div class='prof-card'><h4>Learning Workflow</h4>
            <p>Preluma combines Brain Brief, quiz, Mistake Clinic, UltraTutor, class questions, and readiness score.</p></div>
          <div class='prof-card'><h4>Data Structure Thinking</h4>
            <p>Topic packs use nested dictionaries, aliases, schema validation, and normalized data flow.</p></div>
          <div class='prof-card'><h4>Teacher Value</h4>
            <p>Teacher Studio demonstrates analytics for readiness and weak skill detection.</p></div>
          <div class='prof-card'><h4>Wikipedia Real Data</h4>
            <p>Unknown topics are fetched through the Wikipedia API using Python requests.</p></div>
          <div class='prof-card'><h4>Smart QnA</h4>
            <p>Questions are answered from curated topic data and fetched summary text, with examples and reliability notes.</p></div>
          <div class='prof-card'><h4>Claude AI Integration</h4>
            <p>UltraTutor connects to the Claude API for intelligent, topic-aware answers when an API key is provided.</p></div>
          <div class='prof-card'><h4>Future Potential</h4>
            <p>The roadmap can grow into login, database, PDF notes, retrieval, and full AI tutor support.</p></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    concepts_df = pd.DataFrame({
        "Concept": [
            ...
            "Requests API", "Real Data Fallback", "Claude API",
        ],
        "Use in Project": [
            ...
            "Power UltraTutor with intelligent AI answers",
        ],
    })
    ...
    if errors:
        ...
        st.success("Topic data validation passed for all curated packs.")
...
def demo_guide():
    ...
    steps = [
        "Say: Preluma is a Python-based pre-class learning assistant with curated topics, Wikipedia real-data fallback, and Claude AI integration.",
        "Show the problem: students attend lectures unprepared.",
        "Select Quantum Mechanics or Machine Learning.",
        "Click Start Pre-Class Mission.",
        "Show Brain Brief with clear definition, example, and common mistake.",
        "Take the quiz and show Mistake Clinic with skill detection.",
        "Ask UltraTutor any question and show AI-powered structured answer.",
        "Show Teacher Studio with readiness analytics.",
        "Show Evidence Board with Python concepts used.",
    ]
    for step in steps:
        st.write(f"- {step}")
...
def roadmap():
    ...
    st.code(
        "Current:    Python + Streamlit + curated topic packs + Claude AI\n"
